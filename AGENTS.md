...
**Objetivo de portfólio**: Demonstrar domínio em Java backend (arquitetura em camadas pragmática, Spring Data JPA na camada application (ver ADR-009), TDD, Strategy Pattern, Scheduled Jobs, Analytics via queries otimizadas) com frontend React moderno como vitrine.
...
│   │   │   ├── domain/
│   │   │   │   ├── model/          # Records de domínio anêmicos
│   │   │   │   ├── repository/     # (Omitido intencionalmente - ver ADR-009)
│   │   │   │   └── service/        # Serviços de domínio se houver
│   │   │   ├── application/
│   │   │   │   ├── usecase/        # Casos de uso usando Spring Data JPA (ADR-009)
│   │   │   │   └── dto/            # Records de request/response
│   │   │   ├── infrastructure/
│   │   │   │   ├── persistence/    # Entities JPA + Spring Data Repositories
...
### 3. Evitar exposição de entidades JPA nos controllers
- Controller recebe DTO → chama UseCase → UseCase manipula Spring Data JPA (entidades e repositórios) → retorna DTO de resposta
- A camada application usa entidades e repositórios JPA diretamente (arquitetura em camadas pragmática, Spring Data JPA na camada application, ver ADR-009).
...
**Nota de Segurança**: Rate limiting é best-effort. O IP do cliente vem do primeiro entry do X-Forwarded-For e pode ser spoofado por atacante determinado — aceitável para o threat model deste app de portfólio. Decisão: priorizar bucketing funcional por usuário sobre resistência a spoofing, evitando lockout global.
...
### ADR-009: Arquitetura em camadas pragmática (não Clean Architecture estrita)
**Contexto**: O domain/model é composto de records anêmicos, sem lógica de negócio. A camada application usa Spring Data JPA diretamente (repositórios e entidades), aproveitando lazy loading e dirty checking.
**Decisão**: Adotar arquitetura em camadas pragmática ("Onion pragmática"). Spring Data JPA na camada application é aceito. Camada de ports/adapters e mappers Record↔Entity deliberadamente OMITIDA — para um domínio anêmico, ela só adicionaria boilerplate e complexidade acidental sem proteger nenhuma lógica de negócio.
**Consequências**: (+) Menos código, menos boilerplate, testes estáveis, adesão ao YAGNI. (-) Acoplamento da application ao Spring Data — aceito conscientemente. Revisitar SE o domínio passar a ter comportamento rico.
**Status**: Aceita
...
13. **taskAppliesOnDate() — filtro central**: `GetDayScheduleUseCase.taskAppliesOnDate(TaskJpaEntity, LocalDate)` é o único ponto que decide se uma task aparece num dado dia. DAY_OF_WEEK: compara `task.dayOfWeek == date.getDayOfWeek()`. DAY_OF_MONTH clampa no último dia do mês quando o dia agendado excede os dias do mês (ex: dia 31 dispara em 28/fev). Decisão consciente — evita que tarefas mensais sumam silenciosamente (ADR/regra de produto). Reutilizado por `GetDailyProgressUseCase` e `StreakCalculationService` via referência estática — nunca duplicar essa lógica.
