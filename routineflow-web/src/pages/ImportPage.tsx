  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? 'border-[#2F8BFF] bg-[#2F8BFF]/10'
          : 'border-[#26262A] bg-[#141416] hover:border-[#34343A]',
      )}
    >
      <div className="flex items-start gap-3">
        <span className={cn('mt-0.5 shrink-0', selected ? 'text-[#2F8BFF]' : 'text-[#8C8A88]')}>
          {icon}
        </span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-[#F4F2EF]">{title}</span>
            {badge && (
              <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[#2F8BFF]/20 text-[#2F8BFF]">
                {badge}
              </span>
            )}
          </div>
          <p className="text-xs text-[#8C8A88] mt-1 leading-relaxed">{description}</p>
        </div>
      </div>
    </button>
  )
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <header className="mb-8">
        <h1 className="text-2xl font-light text-[#F4F2EF] tracking-tight">Importar rotina</h1>
        ...
        {file ? (
          ...
          <div className="flex items-center gap-3 justify-center">
            ...
            <div className="text-left min-w-0">
              ...
              <p className="text-xs text-[#8C8A88] mt-0.5">{formatBytes(file.size)}</p>
            </div>
            <button
              type="button"
              onClick={(e) => {
          ...
          /* Upload prompt */
          <>
            <UploadCloud size={48} className="mx-auto text-[#8C8A88] mb-4" strokeWidth={1.25} />
            <p className="text-sm font-medium text-[#F4F2EF]">Arraste seu arquivo aqui</p>
            <p className="text-xs text-[#8C8A88] mt-1">
              ou{' '}
              <span className="text-[#2F8BFF] font-medium">clique para selecionar</span>
            </p>
            <p className="text-[10px] text-[#34343A] mt-3">.yaml · .yml · .txt</p>
          </>
        )}
      </div>
      ...
      {file && (
        <Button
          ...
          disabled={isPending}
          className="w-full mt-4 bg-[#2F8BFF] hover:bg-[#4F9DFF] text-white font-medium h-11 rounded-xl transition-colors disabled:opacity-60"
        >
          {isPending ? (
      ...
      <section className="mt-10">
        ...
        <pre className="rounded-xl bg-[#141416] border border-[#26262A] p-4 text-xs font-mono text-[#8C8A88] overflow-x-auto leading-relaxed whitespace-pre">
          {EXAMPLE_YAML}
        </pre>
        <p className="text-[11px] text-[#34343A] mt-3">
          Dias suportados: MONDAY · TUESDAY · WEDNESDAY · THURSDAY · FRIDAY · SATURDAY · SUNDAY
        </p>
        <p className="text-xs text-[#8C8A88] mt-3">
          Vem do HabitNow?{' '}
          <button
      ...
      <Dialog open={showModeModal} onOpenChange={setShowModeModal}>
        <DialogContent className="bg-[#1C1C1F] border border-[#26262A] rounded-2xl max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-[#F4F2EF] text-base font-medium">
              Como importar?
            </DialogTitle>
            <DialogDescription className="text-[#8C8A88] text-sm">
          ...
          <div className="flex gap-2 mt-2">
            <Button
              ...
              className="flex-1 text-[#8C8A88] hover:text-[#F4F2EF] hover:bg-[#26262A] h-10 rounded-xl"
            >
              Cancelar
            </Button>
            <Button
              onClick={handleConfirmImport}
              className="flex-1 bg-[#2F8BFF] hover:bg-[#4F9DFF] text-white h-10 rounded-xl"
            >
              Confirmar
            </Button>
          </div>
